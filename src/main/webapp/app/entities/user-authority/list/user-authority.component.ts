...
export class UserAuthorityComponent implements OnInit {
  ...
  trackId(index: number, item: IUserAuthority): number {
    return item.id!;
  }

  delete(userAuthority: IUserAuthority): void {
    const modalRef = this.modalService.open(UserAuthorityDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.userAuthority = userAuthority;
  ...
  protected onSuccess(data: IUserAuthority[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/user-authority'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
