...
export class FunctionalityComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.functionalityService
      ...
      .subscribe(
        (res: HttpResponse<IFunctionality[]>) => {
  ...
  trackId(index: number, item: IFunctionality): number {
    return item.id!;
  }

  delete(functionality: IFunctionality): void {
    const modalRef = this.modalService.open(FunctionalityDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IFunctionality[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
