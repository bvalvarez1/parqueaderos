...
export class ContractService {
  ...
  protected convertDateFromClient(contract: IContract): IContract {
    return Object.assign({}, contract, {
      initDate: contract.initDate?.isValid() ? contract.initDate.format(DATE_FORMAT) : undefined,
      endDate: contract.endDate?.isValid() ? contract.endDate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((contract: IContract) => {
        contract.initDate = contract.initDate ? dayjs(contract.initDate) : undefined;
        contract.endDate = contract.endDate ? dayjs(contract.endDate) : undefined;
      });
    }
    return res;
  }
